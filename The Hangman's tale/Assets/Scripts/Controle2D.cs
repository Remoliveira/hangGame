...
namespace HangmansTale
{
    ...
    public class Controle2D : MonoBehaviour
    {
        ...
        public float tempoDesdeChaoRef = 0.2f;
        #endregion
        ...
        private void Update()
        {
            tempoDesdeChao -= Time.deltaTime;
            if (_estaNoChao)
            {
                tempoDesdeChao = tempoDesdeChaoRef;
            }
        }
        ...
        {
            if(!_estaNoChao && _rigidbody2D.velocity.y> 0)
            {
                _rigidbody2D.velocity = new Vector2(_rigidbody2D.velocity.x, _rigidbody2D.velocity.y * 0.25f);
            }
        }
